...
/** @internal @brief Dump buffer to binary file for Rust numerical tests.
 *  Enabled at runtime by setting VLFEAT_SIFT_DUMP=1 (writes to /tmp/vl_sift_*.bin).
 */
static void
vl_sift_dump (const char *tag, vl_sift_pix const *data, vl_size nel)
{
  if (getenv ("VLFEAT_SIFT_DUMP") == NULL) return;
  char fname [256] ;
  snprintf (fname, sizeof(fname), "/tmp/vl_sift_%s.bin", tag) ;
  FILE *f = fopen (fname, "wb") ;
  if (f) {
    fwrite (data, sizeof(vl_sift_pix), nel, f) ;
    fclose (f) ;
  }
}
...
vl_sift_process_first_octave (VlSiftFilt *f, vl_sift_pix const *im)
{
  ...
  vl_sift_dump ("octave_first_base", octave, (vl_size)w * h) ;
  ...
  vl_sift_dump ("octave_first_full", octave, (vl_size)w * h * (s_max - s_min + 1)) ;
...
vl_sift_detect (VlSiftFilt * f)
{
  ...
  vl_sift_dump ("dog", f->dog, (vl_size)w * h * (s_max - s_min)) ;
...
#define SAVE_BACK                                                       \
  ...
  vl_sift_dump ("grad", f->grad, (vl_size)w * h * 2 * (s_max - s_min)) ;
}
